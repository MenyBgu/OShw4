...
public class Q1 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < A.length; i++) {
			for (int j = 0; j < A[0].length; j++) {
				System.out.println("Enter matrix Allocation index: [" + i+"]" + "["+ j+"]");
				A[i][j] = sc.nextInt();
			}
		}
		for (int i = 0; i < A.length; i++) {
			for (int j = 0; j < A[0].length; j++) {
				System.out.println("Enter matrix Claim index: [" + i+"]" + "["+ j+"]");
				C[i][j] = sc.nextInt();
			}
		}
		...
		for(int i=0;i<Resources;i++){
			...
			System.out.println("Minimum resources from type R"+i+" requiered for safe running: "+sum);
		}*/
